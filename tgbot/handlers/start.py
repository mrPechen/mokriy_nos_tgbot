...
from tgbot.keyboards.inline import menu_admin, menu_user
...
async def user_start(message: Message):
    ...
    if user_in_db:
        ...
        await db_commands.get_or_create_user(user_id=message.from_user.id,
        ...
        if user in admins:
            await message.answer(text=f'Привет, {message.from_user.full_name}!\n'
                                 ...
                                 reply_markup=await menu_admin())
        else:
            await message.answer(text=f'Привет, {message.from_user.full_name}!\n'
                                      f'Время заботы \u2764' + '\n'
                                                               f'Выбери к кому хочешь записаться \uE22F',
                                 reply_markup=await menu_user())
