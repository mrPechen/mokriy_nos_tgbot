...
from tgbot.services.db_commands import vet_groomer_category_count, vet_groomer_category_filter
from pytz import timezone

calendar_callback = CallbackData('calendar_callback', 'act', 'year', 'month', 'day', 'hour', 'minutes', 'service_id')


async def calendar_list(year: int = datetime.now().year, month: int = datetime.now().month, service_id: int = None):
    ignore_callback = calendar_callback.new("IGNORE", year, month, 0, 0, 0, 0)
    ...
    for week in month_calendar:
        ...
        for day in week:
            ...
            if day == 0 or await vet_groomer_category_count(date=date(int(year), int(month), int(day))) >= 18:
                markup.insert(
                    InlineKeyboardButton(
                        text=' ',
                        callback_data=ignore_callback
            ...
            markup.insert(
                InlineKeyboardButton(
                    text=str(day),
                    callback_data=calendar_callback.new("DAY", year, month, day, 0, 0, service_id)
                )
            )
    ...
    if month == datetime.now().month:
        ...
        markup.insert(
            InlineKeyboardButton(
                text='<',
                callback_data=calendar_callback.new("PREV-MONTH", year, month, day, 0, 0, service_id)
            )
        )
    markup.insert(
        InlineKeyboardButton(
    ...
    markup.insert(
        InlineKeyboardButton(
            text='>',
            callback_data=calendar_callback.new("NEXT-MONTH", year, month, day, 0, 0, service_id)
        )
    )
    return markup


async def time_list(year: int, month: int, day: int, service_id: int):
    ignore_callback = calendar_callback.new("IGNORE", year, month, 0, 0, 0, 0)
            ...
            markup.insert(
                InlineKeyboardButton(
                    ...
                    callback_data=calendar_callback.new("TIME", year, month, day, time.split('-')[0],
                                                        time.split('-')[1], service_id),
                )
            )
        markup.insert(
            InlineKeyboardButton(
                text='Назад',
                callback_data=calendar_callback.new("Back_to_calendar", year, month, day, 0, 0, service_id)
            )
        )
    else:
        for time in times:
